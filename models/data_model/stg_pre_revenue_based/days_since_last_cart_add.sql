...
datediff(day, date(first_value(timestamp) over(partition by main_id order by timestamp desc)), date({{get_end_timestamp()}})) as days_since_last_cart_add
from cte_id_stitched_product_added
where timestamp >= '{{ var('start_date') }}' and timestamp <= '{{ var('end_date') }}' and main_id is not null
group by main_id, timestamp