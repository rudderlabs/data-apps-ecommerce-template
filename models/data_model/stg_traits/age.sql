with cte_id_stitched_identifies as (
    ...
    {{ var('col_ecommerce_identifies_birthday')}}, 
...
select distinct {{ var('main_id')}}, 
    first_value({{get_age_from_dob( var('col_ecommerce_identifies_birthday'))}}) over(
        ...
        order by case when {{ var('col_ecommerce_identifies_birthday')}} is not null and  {{ var('col_ecommerce_identifies_birthday')}} != '' then 2 else 1 end desc, {{ var('col_ecommerce_identifies_timestamp')}} desc
    ) as age
